...
export default function PlayerForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      playerAPI.getById(id).then((res) => {
        const p = res.data;
        setForm({ ...p, jerseyNumber: p.jerseyNumber || "", dateOfBirth: p.dateOfBirth ? p.dateOfBirth.split("T")[0] : "", socialLinks: p.socialLinks || { instagram: "", twitter: "", facebook: "" } });
      });
    }
  }, [id, isEdit]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      isEdit ? await playerAPI.update(id, form) : await playerAPI.create(form);
      ...
      <form onSubmit={handleSubmit} className="max-w-[900px]">
        ...
        <div className="bg-[#111113] border border-white/[0.06] rounded-lg p-6 mb-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className={labelClass}>Jersey Number</label><input name="jerseyNumber" type="number" value={form.jerseyNumber} onChange={handleChange} required className={inputClass} /></div>
            <div><label className={labelClass}>Position</label>
              <select name="position" value={form.position} onChange={handleChange} className={inputClass}>
                <option value="Guard">Guard</option><option value="Forward">Forward</option><option value="Center">Center</option>
              </select>
            </div>
            <div><label className={labelClass}>Status</label>
              <select name="status" value={form.status} onChange={handleChange} className={inputClass}>
                <option value="active">Active</option><option value="inactive">Inactive</option><option value="injured">Injured</option><option value="retired">Retired</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#111113] border border-white/[0.06] rounded-lg p-6 mb-4">
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div><label className={labelClass}>Weight</label><input name="weight" value={form.weight} onChange={handleChange} placeholder="e.g. 85kg" className={inputClass} /></div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className={labelClass}>Previous Team</label><input name="previousTeam" value={form.previousTeam} onChange={handleChange} className={inputClass} /></div>
          </div>
        </div>
