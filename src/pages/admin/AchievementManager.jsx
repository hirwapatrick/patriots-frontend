...
export default function AchievementManager() {
  ...
  const [form, setForm] = useState({ year: new Date().getFullYear(), title: "", description: "", image: "", category: "", displayOrder: 0, status: "published" });

  useEffect(() => { loadAchievements(); }, []);
  const loadAchievements = () => { achievementAPI.getAllAdmin().then((res) => setAchievements(res.data)); };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      editingId ? await achievementAPI.update(editingId, form) : await achievementAPI.create(form);
      resetForm(); loadAchievements();
    } catch (err) { alert(err.response?.data?.message || "Save failed"); }
  };

  const handleEdit = (a) => {
    setForm({ year: a.year, title: a.title, description: a.description || "", image: a.image || "", category: a.category || "", displayOrder: a.displayOrder || 0, status: a.status });
    setEditingId(a._id);
  };

  const handleDelete = async (id) => {
    if (!window.confirm("Delete this achievement?")) return;
    await achievementAPI.delete(id); loadAchievements();
  };

  const resetForm = () => { setForm({ year: new Date().getFullYear(), title: "", description: "", image: "", category: "", displayOrder: 0, status: "published" }); setEditingId(null); };
      ...
      <div className="bg-[#111113] border border-white/[0.06] rounded-lg p-6 mb-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-[1fr_2fr] gap-4 mb-4">
            <div><label className={labelClass}>Year</label><input name="year" type="number" value={form.year} onChange={handleChange} required className={inputClass} /></div>
            <div><label className={labelClass}>Title</label><input name="title" value={form.title} onChange={handleChange} required className={inputClass} /></div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-5">
            <div><label className={labelClass}>Category</label><input name="category" value={form.category} onChange={handleChange} placeholder="e.g. League, BAL" className={inputClass} /></div>
            <div><label className={labelClass}>Display Order</label><input name="displayOrder" type="number" value={form.displayOrder} onChange={handleChange} className={inputClass} /></div>
        ...
        <div className="bg-[#111113] border border-white/[0.06] rounded-lg overflow-hidden">
          <table className="w-full text-left">
            <thead><tr className="border-b border-white/[0.06]">
              ...
              <th className="px-5 py-3 text-[0.65rem] font-semibold uppercase tracking-[1.5px] text-gray-500">Title</th>
              <th className="px-5 py-3 text-[0.65rem] font-semibold uppercase tracking-[1.5px] text-gray-500">Category</th>
            ...
            <tbody>
              {achievements.map((a) => (
                <tr key={a._id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  <td className="px-5 py-3 font-heading text-[1.2rem] text-red">{a.year}</td>
                  <td className="px-5 py-3 text-[0.85rem] font-medium">{a.title}</td>
                  <td className="px-5 py-3 text-[0.8rem] text-gray-400">{a.category || "—"}</td>
                  <td className="px-5 py-3"><span className={`text-[0.65rem] font-semibold uppercase tracking-[1px] px-2 py-0.5 rounded ${a.status === "published" ? "bg-green-500/10 text-green-500" : "bg-white/[0.04] text-gray-500"}`}>{a.status}</span></td>
                  <td className="px-5 py-3 text-right">
                    <div className="flex justify-end gap-1.5">
                      <button onClick={() => handleEdit(a)} className="flex items-center gap-1 px-2.5 py-1 bg-white/[0.04] text-gray-400 text-[0.65rem] font-semibold rounded border border-white/[0.06] hover:text-white hover:bg-white/[0.08] transition-all duration-150"><Pencil size={10} /></button>
                      <button onClick={() => handleDelete(a._id)} className="flex items-center gap-1 px-2.5 py-1 bg-white/[0.04] text-gray-400 text-[0.65rem] font-semibold rounded border border-white/[0.06] hover:text-red hover:bg-red/10 transition-all duration-150"><Trash2 size={10} /></button>
                    </div>
                  </td>
                </tr>
              ))}
